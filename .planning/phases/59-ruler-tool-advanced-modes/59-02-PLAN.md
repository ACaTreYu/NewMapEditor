...
4. **handleMouseDown double-click detection** — Use `e.detail === 2` to detect double-click in path mode. On double-click:
   - Finalize the path (don't add the double-click point as a new waypoint — it duplicates the single-click point already added)
   ...
   - Actually, simpler approach: on double-click, just set active to false. The measurement persists in Zustand for status bar display. The drawUiLayer should render based on rulerMeasurement data (not just rulerStateRef.active) for completed paths.

   **Revised approach for completed path rendering:** When path is "completed" (double-click), set `rulerStateRef.current.active = false`. Store the completed waypoints in `rulerMeasurement.waypoints`. In `drawUiLayer`, check if `rulerMeasurement?.mode === RulerMode.PATH && rulerMeasurement.waypoints?.length > 0` to render a completed path even when `rulerStateRef.current.active === false`. This way:
...
1. Import `RulerMode` from globalSlice. Import Lucide icons for mode buttons: `LuMinus` (line), `LuRectangleHorizontal` (rectangle), `LuRoute` (path), `LuCircle` (radius). If `LuRoute` is not available in the existing lucide import, use `LuWaypoints` or `LuCornerDownRight`. Check what's available in the existing imports.
...
**Important:** Check that the Lucide icon names are correct — verify which icons are already imported in ToolBar.tsx and reuse consistent naming. The icons `LuMinus`, `LuRectangleHorizontal`, and `LuCircle` are already imported in ToolBar.tsx. For path, check if `LuRoute` exists in react-icons/lu; if not, use `LuCornerDownRight` or just a text label "P".
  </action>
  <verify>Run `npx tsc --noEmit` — no type errors. Run `npm run electron:dev`:
