...
<task type="auto">
  ...
  **Step 10: Wire RAF into pan mousemove.** In `handleMouseMove`, inside the `isDragging && panStartRef.current` branch (currently lines 927-936), AFTER the last CSS transform assignment (gridLayerRef line) and BEFORE the `return` statement, add:
  ```typescript
