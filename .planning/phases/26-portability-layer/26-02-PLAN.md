...
must_haves:
  truths:
    ...
    - "App works with either Electron or browser FileService adapter"
    - "FileServiceProvider wraps App in main.tsx with runtime platform detection"
  artifacts:
...
<objective>
Extract map load/save logic from App.tsx into a MapService, refactor App.tsx to use FileService via Context, and wire the FileServiceProvider in main.tsx with runtime platform detection.

Purpose: Complete the portability layer by removing ALL direct `window.electronAPI` calls from components and core, making the app work with either Electron or browser adapters.
...
a) Remove the `isElectron` check at line 14:
```typescript
...
d) Replace `handleOpenMap` (lines 67-135) with:
```typescript
...
e) Replace `handleSaveMap` (lines 138-194) with:
```typescript
...
**Key details:**
- Use `(window as any).electronAPI` for the detection check since the component code no longer directly accesses `window.electronAPI` (only the adapter does). The `as any` avoids needing the global type declaration in contexts where it might not exist.
- The `isElectron` check is now in exactly ONE place: main.tsx. All downstream code is platform-agnostic.
- ElectronFileService is instantiated only when running in Electron. BrowserFileService is instantiated only when running in a browser. The unused implementation is never constructed.
  </action>
  <verify>
...
3. Verify the `isElectron` detection correctly identifies the Electron environment
  </verify>
  <done>
main.tsx wraps App in FileServiceProvider with runtime platform detection. The app starts in Electron with ElectronFileService and would use BrowserFileService in a web context. All file operations work unchanged from user's perspective.
  </done>
</task>
...
3. `window.electronAPI` only appears in:
   ...
   - `src/main.tsx` (platform detection only)
...
- App.tsx works with either Electron or browser file adapter (via Context)
