...
<objective>
Migrate all component CSS files containing var(--win98-*) references to modern semantic design tokens.
...
Output: All 8 component CSS files with var(--win98-*) references updated to use only semantic design tokens. Zero var(--win98-*) references in entire codebase. Components display modern minimalist styling.

NOTE: The following 6 component CSS files were audited and contain ZERO var(--win98-*) references, so they require no migration and are excluded from this plan:
- src/components/TabbedBottomPanel/TabbedBottomPanel.css (clean)
- src/components/TilesetPanel/TilesetPanel.css (clean)
- src/components/TilePalette/TilePalette.css (clean)
- src/components/MapSettingsPanel/MapSettingsPanel.css (clean)
- src/components/RightSidebar/RightSidebar.css (clean)
- src/components/MapCanvas/MapCanvas.css (clean)
</objective>
