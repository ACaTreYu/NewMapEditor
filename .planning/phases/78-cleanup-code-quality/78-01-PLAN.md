...
**2. MapCanvas.tsx line ~1968 — Prefix unused `e` parameter with underscore in `handleMouseUp`:**
The function `const handleMouseUp = (e: React.MouseEvent) => {` at line ~1968 does not use the `e` parameter in its body. This function is bound as a React event handler via `onMouseUp={handleMouseUp}` (line ~2540), so React passes the event automatically. The parameter cannot be removed. Prefix with underscore:
```typescript
const handleMouseUp = (_e: React.MouseEvent) => {
```

**3. MapCanvas.tsx line ~2067 — Prefix unused `e` parameter with underscore in `handleMouseLeave`:**
The function `const handleMouseLeave = (e: React.MouseEvent) => {` at line ~2067 does not use the `e` parameter in its body. This function is bound as a React event handler via `onMouseLeave={handleMouseLeave}` (line ~2541), so React passes the event automatically. The parameter cannot be removed. Prefix with underscore:
```typescript
