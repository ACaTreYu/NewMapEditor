...
<task type="auto">
  ...
  <action>
    ...
    **E. Replace box-shadow blur effects** — replace decorative box-shadows (with blur radius > 0) with Win98 bevel borders or remove.
    ...
    **MapSettingsDialog.css:**
    ...
    - Hardcoded bevel colors (#0a0a0a, #808080, #ffffff, #dfdfdf) in box-shadows throughout: replace with CSS variable equivalents (var(--win98-ButtonDkShadow), var(--win98-ButtonShadow), var(--win98-ButtonHighlight), var(--win98-ButtonLight)). This makes bevels adapt to theme changes.
  ...
  <done>ToolBar, MapCanvas, MapSettingsDialog, Minimap, StatusBar, and TilesetPanel CSS files purged of all modern artifacts and ghost variables replaced.</done>
</task>
...
<task type="auto">
  ...
  <done>All remaining component CSS files purged of modern artifacts. Zero border-radius, zero transitions, zero rgba(), zero opacity on controls, zero filter effects, zero ghost variables across entire component CSS codebase.</done>
</task>
