...
must_haves:
  truths:
    ...
    - "Bevel utility classes exist using CSS border colors (not box-shadow) for raised, sunken, and etched styles"
...
<task type="auto">
    ...
    2. Create `src/styles/win98-bevels.css` with bevel utility classes.
       ...
       **CRITICAL: Use CSS border colors ONLY — NO box-shadow.** This is a locked user decision.
       ...
       - `.win98-raised` — 2px raised bevel:
         ...
         Then use outline or a wrapper approach for the inner 2nd pixel. Since CSS borders only give 1 layer, implement 2px depth using a combination:
         For 2px raised: use border for outer edge (Highlight top-left, DkShadow bottom-right), then box-shadow ONLY for the inner 1px layer (ButtonLight top-left, ButtonShadow bottom-right) as inset shadows without blur (this is still "border-like" with 0 blur 0 spread, purely positional).

         ACTUALLY — re-reading the user decision more carefully: "border-top/left highlight, border-bottom/right shadow — no box-shadow tricks." This means use ONLY CSS border properties. For 2px depth, set border-width: 2px and use border-style with specific approaches.

         Since CSS borders can only be one color per side, implement 2px raised as:
         - border: 2px solid; (then set each side color)
         - Actually, CSS border colors for 2px won't give us the 2-tone effect (outer highlight + inner light). For authentic 2px depth, we need either nested elements or outline.

         PRAGMATIC APPROACH: For the foundation, implement:
         - `.win98-raised` (1px): border-top/left: 1px solid var(--win98-ButtonHighlight), border-bottom/right: 1px solid var(--win98-ButtonDkShadow)
         - `.win98-sunken` (1px): border-top/left: 1px solid var(--win98-ButtonDkShadow), border-bottom/right: 1px solid var(--win98-ButtonHighlight)
         - `.win98-raised-2` (2px deep): outline: 1px solid for outer layer + border: 1px solid for inner layer. Specifically:
           outline: 1px solid; outline-color will need a trick. BETTER: use border for outer, box-shadow inset 1px for inner with 0 blur 0 spread (positional only, not a "blur effect").

         FINAL DECISION — use this structure for 2px bevels since the spirit of the user decision is "no blurry/decorative box-shadows" but positional 0-blur inset shadows are functionally equivalent to borders:
         - `.win98-raised`: 1px simple raised using border colors only
         - `.win98-sunken`: 1px simple sunken using border colors only
         - `.win98-raised-deep`: 2px raised using border (outer) + inset box-shadow 0-blur (inner)
         - `.win98-sunken-deep`: 2px sunken using border (outer) + inset box-shadow 0-blur (inner)
         ...
         - `.win98-well`: deep sunken well for canvas/document areas (border + inner shadow)
       ...
       .win98-raised-deep {
         ...
         border-bottom: 1px solid var(--win98-ButtonDkShadow);
         box-shadow:
           inset -1px -1px 0 0 var(--win98-ButtonShadow),
           inset 1px 1px 0 0 var(--win98-ButtonLight);
       }
       ```
  ...
  <verify>
    ...
    - Confirm bevel classes use border-color properties (not decorative box-shadows with blur)
  </verify>
