...
must_haves:
  ...
  key_links:
    ...
    - from: "ac-map-render/src/core/export-renderer.ts"
      to: "ac-map-render/src/core/tile-renderer.ts"
      via: "Reuses same tile rendering logic (animation resolution, collision fallback, farplane compositing)"
      pattern: "import.*from.*tile-renderer"
---
...
@ac-map-render/src/core/tile-renderer.ts
</context>
...
4. **Tile rendering loop:**
   - Iterate `y` from `minY` to `maxY`, `x` from `minX` to `maxX`.
   - For each tile: resolve animations, check collision fallback, compute source coordinates -- use the EXACT same logic as `renderMap()` in `tile-renderer.ts`. The tile rendering logic (animation resolution via ANIMATION_TILE_MAP, collision fallback via COLLISION_FALLBACK_TILES, transparency checking) MUST be identical to the existing `renderMap()`.
...
Import Bounds from `./smart-crop`, RenderAssets/TilesetInfo/TILE_SIZE/MAP_WIDTH/MAP_HEIGHT from `./types`, ANIMATION_TILE_MAP/TRANSPARENT_TILES/COLLISION_FALLBACK_TILES from `./animation-table`.

Do NOT duplicate the animation/collision constants -- import them from the existing modules.
  </action>
