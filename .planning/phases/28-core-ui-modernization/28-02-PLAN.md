...
must_haves:
  truths:
    - "All hardcoded font-size px values replaced with var(--font-size-*) tokens"
    - "All hardcoded sub-8px spacing values replaced with var(--space-*) tokens"
    - "Hardcoded font-weight 600 replaced with var(--font-weight-semibold)"
...
Output: All 14 component CSS files use exclusively var() references for spacing, font sizes, and font weights (with acceptable exceptions for structural 1px borders, width/height dimensions, and intrinsic sizing values).
</objective>
...
- `.toolbar-button` padding: `3px` → `var(--space-0_25)` (rounding down from 3 to 2 is tight; alternatively leave 3px as-is since there's no --space-0_375 token. Use `var(--space-0_25)` — the 1px reduction on a 22px button is imperceptible.)
...
**Additional note on 3px values:** The toolbar button has `padding: 3px`. Since there's no 3px token, use `var(--space-0_25)` (2px). The visual difference is negligible on a 22px button. If this causes visual issues, the executor can keep 3px with a `/* no token for 3px */` comment.
