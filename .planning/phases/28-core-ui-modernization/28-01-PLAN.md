...
must_haves:
  truths:
    - "Zero hardcoded hex color values (#xxxxxx) in any component CSS file"
    - "Zero hardcoded rgba() values in any component CSS file"
    - "All new semantic tokens defined in variables.css"
    - "Compact spacing tokens (sub-8px) available for component use"
    - "Typography scale tokens cover all font sizes used in components"
  artifacts:
