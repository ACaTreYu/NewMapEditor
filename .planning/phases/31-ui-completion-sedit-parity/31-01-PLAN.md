...
must_haves:
  truths:
    - "New maps created via createDefaultHeader() use SEdit-matching defaults for all 7 corrected fields (laserDamage=2, specialDamage=2, rechargeRate=2, holdingTime=15, maxSimulPowerups=12, name='New Map', description='New map')"
...
<objective>
Fix all 7 SEdit default value mismatches (5 numeric fields + name + description), consolidate SETTING_CATEGORIES from 10 tabs to 5 groups, and document the binary format "missles" typo in MapParser.ts.
...
<task type="auto">
  ...
  <action>
**In `src/core/map/types.ts` — Fix 7 defaults in `createDefaultHeader()` (5 numeric + name + description):**
...
(This is a comment, not runtime console.log — keep it as documentation.)
  ...
  <done>createDefaultHeader() returns SEdit-matching defaults for all 7 corrected fields (5 numeric + name + description). SETTING_CATEGORIES has exactly 5 entries. All 53 GAME_SETTINGS have correct category assignments. GameSetting interface includes optional subcategory field.</done>
</task>
...
<success_criteria>
- SEdit default values match exactly (7 fields corrected: 5 numeric + name + description)
