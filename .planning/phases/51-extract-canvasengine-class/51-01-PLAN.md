...
must_haves:
  ...
  key_links:
    ...
    - from: "src/components/MapCanvas/MapCanvas.tsx"
      to: "src/core/canvas/CanvasEngine.ts"
      via: "drawMapLayer calls engine.drawMapLayer(), immediatePatchTile calls engine.patchTile()"
      pattern: "engine(Ref\\.current)?\\.(drawMapLayer|patchTile|blitToScreen)"
---
...
export class CanvasEngine {
  ...
  drawMapLayer(screenCanvas: HTMLCanvasElement, map: MapData, viewport: Viewport, animFrame: number): void
...
5. **`drawMapLayer(screenCanvas, map, viewport, animFrame)`** — Extract from lines 261-326. Handles both full buffer rebuild (when prevTiles is null or tileset changed) and incremental patch (diff and update only changed tiles). Then blits buffer to screen. The `screenCanvas` parameter is needed for the first call before attach stores it, but normally uses stored screen context. Actually, simplify: `drawMapLayer` accepts `map`, `viewport`, `animFrame` and operates on stored contexts from `attach()`.
...
7. **`patchTile(tileX, tileY, tile, viewport, animFrame)`** — Extract from lines 246-256 (`immediatePatchTile`). Patches single tile on buffer, updates prevTiles snapshot, blits to screen.

8. **`patchAnimatedTiles(map, viewport, animFrame, canvasWidth, canvasHeight)`** — Extract from lines 786-832 (animation tick useEffect body). Patches only animated tiles in visible area on the buffer, blits to screen. Returns boolean indicating whether any animated tiles were updated.
...
- `detached` flag: checked at start of `drawMapLayer`, `blitToScreen`, `patchTile`, `patchAnimatedTiles` — early return if detached.
...
Run `npm run typecheck` — zero TypeScript errors. The CanvasEngine class should compile cleanly with proper types for all methods and properties.
  ...
  <done>
`src/core/canvas/CanvasEngine.ts` exists with attach/detach lifecycle, renderTile, drawMapLayer (full rebuild + incremental patch), blitToScreen, patchTile, and patchAnimatedTiles methods. Barrel exports work. TypeScript compiles with zero errors.
  </done>
</task>
...
11. **Update `handleToolAction`** (lines 1304-1352): The multi-tile stamp path (lines 1328-1337) directly accesses `mapBufferCtxRef.current` and calls `renderTile()`. Replace with:
```typescript
for (const t of tiles) {
  if (t.x >= 0 && t.x < MAP_WIDTH && t.y >= 0 && t.y < MAP_HEIGHT) {
    engineRef.current?.patchTile(t.x, t.y, t.tile, viewport, animFrameRef.current);
  }
}
```
Remove the separate `immediateBlitToScreen(viewport)` call after the loop since `patchTile` blits internally. Actually — this would blit N times for N tiles. Instead, add a `patchTileNoCommit` method that patches the buffer without blitting, then blit once at the end. OR keep the existing pattern of patching buffer directly and blitting once. The simplest approach: add a `patchTileBuffer(tileX, tileY, tile, animFrame)` method that only patches the buffer (no blit), and keep `blitToScreen` as a separate call. Then the multi-tile stamp does:
```typescript
for (const t of tiles) {
  if (t.x >= 0 && t.x < MAP_WIDTH && t.y >= 0 && t.y < MAP_HEIGHT) {
...
```
Add `patchTileBuffer(tileX, tileY, tile, animFrame)` to CanvasEngine in Task 1 — it does the clearRect + renderTile + prevTiles update without blitting.
...
4. Draw with pencil tool — immediate visual feedback, no flicker.
...
8. All tools still work (pencil, fill, wall, line, select, game objects).
  ...
  <done>
MapCanvas.tsx creates CanvasEngine on mount, detaches on unmount, and delegates all pixel operations (drawMapLayer, blitToScreen, patchTile, patchAnimatedTiles) to the engine. No rendering useCallbacks with buffer/context logic remain in MapCanvas. The standalone renderTile function is removed. All refs for buffer management (mapBufferRef, mapBufferCtxRef, prevTilesRef, prevTilesetRef, lastBlitVpRef) are removed from MapCanvas. App behavior is identical to before.
  </done>
</task>
...
- Engine methods: drawMapLayer (full rebuild + incremental patch), blitToScreen, patchTile, patchTileBuffer, patchAnimatedTiles, renderTile
