...
9. **`patchAnimatedTiles(map, viewport, animFrame, canvasWidth, canvasHeight)`** — Extract from lines 786-832 (animation tick useEffect body). Patches only animated tiles in visible area on the buffer, blits to screen. Returns boolean indicating whether any animated tiles were updated. **IMPORTANT:** This method does NOT update `this.prevTiles` for animated tile positions. This is correct — animated tiles change appearance every frame via `animFrame`, but their tile *value* in the map array doesn't change. The incremental patch in `drawMapLayer` compares tile values, not rendered output, so it correctly skips unchanged animated tiles while `patchAnimatedTiles` handles their visual updates.
...
useEffect(() => {
  ...
  // NOTE: Do NOT add `hasAnimated` here — it would cause unnecessary UI redraws every tick.
  // The UI layer only needs redrawing when overlay features (selection, paste, conveyor) exist.
