...
must_haves:
  ...
  key_links:
    - from: "src/components/ToolBar/ToolBar.tsx"
      ...
      via: "variant dropdown setter"
      pattern: "rotateSelectionForDocument.*angle"
...
Rotation executes immediately on variant selection (not a tool mode). Button disabled when no selection or in paste mode (handled by setter early returns).
  </action>
  <verify>
1. `npx tsc --noEmit` passes with zero TypeScript errors
...
6. Manual test: Open map, create 3x5 selection with SELECT tool, click rotate button dropdown and select "90° CW", verify selection becomes 5x3 and tiles rotated correctly
  </verify>
  <done>
